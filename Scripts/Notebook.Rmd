...
In England, [a recent
article](https://doi.org/10.1186/s40834-022-00169-w) revealed changes in
prescribing of contraception between 2019 and 2020 on account of the
SARS-CoV-2 pandemic, and associated restrictions. I will examine if this
is the case in Scotland too.

I have divided contraceptive prescriptions into three classes: LARC,
...
decrease in prescribing of long-acting reversible contraception (LARC),
and emergency contraception due to social restrictions. I also imagine
that an associated uptick in the numbers of oral contraceptive
prescriptions will be dispensed due to these drugs being prescribed in
place of LARC.
...
community from NHS Scotland, geospatial data, and the Scottish Index of
...
```{r ContraceptionTimePeriods, warning=FALSE, message=FALSE, eval=FALSE}
...
TableData %>%
  ...
  tab_header(title = "Mean Quantities of Contraception Dispensed by NHS Scotland", subtitle = "Between October 2015 and July 2020") %>%
  ...
  tab_row_group(label = "LARC", rows = 1:2) %>% 
  tab_row_group(label = "Oral Contraception", rows = 3:4) %>% 
  tab_row_group(label = "Emergency Contraception", rows = 5:6) %>% 
  cols_move(TimePeriodDates, after = TimePeriod) %>%
  fmt_integer(MeanPrescriptionsDispensedPerHealthBoardPerTimePeriod) %>% 
  tab_style(style = list(cell_text(color = "red")), 
            locations = cells_body(columns = PercentageChangePretty, 
                                   rows = PercentageChange < 0)) %>% 
  tab_style(style = list(cell_text(color = "chartreuse4")), 
            locations = cells_body(columns = PercentageChangePretty, 
                                   rows = PercentageChange > 0))  %>% 
  gt_theme_538() %>%
  tab_options(heading.align = "center", row_group.font.weight = "bold", row_group.background.color = "turquoise", row.striping.background_color = "grey90") %>%
  opt_row_striping() %>%
  cols_align(align = "center", columns = c(TimePeriodDates:PercentageChangePretty))
```
...
![](images/Rplot12.png){width="100mm"}
...
and Arran, there is a large overall descrease in contraception
...
(SIMD Viginitile 1), or least deprived (SIMD Viginitile 20), broken down
by Health Board area. These results have been normalised, and are
presented as z-scores.

![](images/Rplot09.png){width="100mm"}
...
1.  *"In Scotland, as in England, contraceptives prescribed in the
    community changed due to Covid-19 lockdowns."*

    -   Analysis of these data support that the same decrease is
        observed. However, subsequent statistical work is required to
        confirm the significance of this finding.

2.  *"In Scotland, contraceptives prescribed in the community during
    Covid-19 lockdowns differed by deprivation."*

    -   The answer to this hypothesis is less clear. It does appear that
        deprivation plays a role in the changes to type and quantity of
        contraceptive prescriptions dispensed during March 2020.
        However, this appears to vary more widely on a Health Board
        basis than purely on deprivation alone. Further analysis,
        perhaps looking at a smaller geographical unit may reveal these
        differences, along with information about differences in Health
        Board policies at the time.
