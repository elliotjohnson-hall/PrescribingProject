...
knitr::opts_chunk$set(echo = TRUE, tidy.opts = list(width.cutoff = 80),tidy = TRUE, error = TRUE)
```

```{r LoadLibraries, message=FALSE, warning=FALSE}
if(!require(pacman))install.packages("pacman")
pacman::p_load("gtExtras", "tidyverse", "lubridate", "readxl", "ggplot2", "gt", "here", "colorspace", "patchwork", "scales", "extrafont")
```
...
LARCPlotScotland <- LARC %>%
  ...
  ggplot(aes(x = PaidDateMonth, y = SumPaidQuantity)) +
  AnnotateLockdowns() +
  geom_col(aes(y = MeanPrescriptionsDispensedPerHealthBoard * ScaleFactor, fill =  NumberHealthBoards)) +
  scale_y_continuous(sec.axis = sec_axis(~ . / ScaleFactor)) +
  geom_col(aes(y = NumberHealthBoards)) +
