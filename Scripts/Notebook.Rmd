...
pacman::p_load("gtExtras", "tidyverse", "lubridate", "readxl", "ggplot2", "gt", "here", "colorspace", "patchwork", "scales")
```
...
```{r}
TabletDeprivationPlot <- SIMDPlot(SIMDContraceptionLookup(Tablet, 20)) +
  labs(title = "1&20 Tab") +
  scale_y_continuous(labels = label_percent(), sec.axis = sec_axis(~ . / 100, labels = label_percent(), name = derive())) +
  geom_line(data = SIMDContraceptionLookup(EmergencyContraception, 20), mapping = aes(y = ContraceptionPerHeadPerMonthPerHealthBoard * 100), colour = "red")
  
LARCDeprivationPlot <- SIMDPlot(SIMDContraceptionLookup(LARC, 1)) +
  labs(title = "1&20 LARC") +
  scale_y_continuous(labels = label_percent()) +
    geom_line(data = SIMDContraceptionLookup(EmergencyContraception, 20), colour = "red")

EmergencyContraceptionDeprivationPlot <- SIMDPlot(SIMDContraceptionLookup(EmergencyContraception, 1)) +
  labs(title = "1&20 EC") +
  scale_y_continuous(labels = label_percent()) +
  geom_line(data = SIMDContraceptionLookup(EmergencyContraception, 20), colour = "red")

DeprivationPlots <- wrap_plots(TabletDeprivationPlot/ LARCDeprivationPlot,EmergencyContraceptionDeprivationPlot) +
  plot_annotation(
