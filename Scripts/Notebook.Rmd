...
knitr::opts_chunk$set(echo = TRUE, tidy.opts = list(width.cutoff = 100), tidy = TRUE, error = TRUE, dev="cairo_pdf")
```

```{r LoadLibraries, message=FALSE, warning=FALSE}
if(!require(pacman))install.packages("pacman")
pacman::p_load("gtExtras", "tidyverse", "lubridate", "readxl", "ggplot2",
...
LARCPlot <- LARC %>%
  ...
  geom_point(mapping = aes(colour = HealthBoardName), shape = 1) +
...
LARCPlotScotland <- LARC %>%
  group_by(PaidDateMonth) %>%
  summarise(SumPaidQuantity = sum(PaidQuantity), NumberHealthBoards = n_distinct(HealthBoardCode), .groups = 'drop') %>%
  mutate(MeanPrescriptionsDispensedPerHealthBoard = (SumPaidQuantity/NumberHealthBoards)) %>% 
  ggplot(aes(x = PaidDateMonth, y = SumPaidQuantity)) +
  AnnotateLockdowns() +
  geom_col(aes(y = MeanPrescriptionsDispensedPerHealthBoard * ScaleFactor, fill =  NumberHealthBoards), width = 41, position = position_identity()) +
annotate("text", x = as_date("2017-01-01"), y = 3000, label = "10", colour = "white") +
  annotate("text", x = as_date("2018-09-01"), y = 3000, label = "12", colour = "white") +
  annotate("text", x = as_date("2021-01-01"), y = 3000, label = "14", colour = "white") +
  scale_y_continuous(sec.axis = sec_axis(~ . / ScaleFactor, name = "Mean LARC Prescriptions Dispensed per Month \n Adjusted for Number of Health Boards")) +
...
TableWrangle <- function(Dataset) {
  Dataset %>%
    ...
    group_by(PaidDateMonth) %>%
    summarise(
      SumPaidQuantity = sum(PaidQuantity),
      NumberHealthBoards = n_distinct(HealthBoardCode),
    ...
    mutate(MeanPrescriptionsDispensedPerHealthBoardPerMonth = ((SumPaidQuantity /
                                                                  NumberHealthBoards) / NumberMonths)) %>%
    group_by(TimePeriod) %>%
    mutate(MeanPrescriptionsDispensedPerHealthBoardPerTimePeriod = (mean(
      MeanPrescriptionsDispensedPerHealthBoardPerMonth
    ))) %>%
    select(
      TimePeriod,
      MeanPrescriptionsDispensedPerHealthBoardPerTimePeriod,
      TimePeriodDates
    ) %>%
    distinct(
      TimePeriod,
    ...
    mutate(PercentageChangePretty = case_when(
...
TableData <- bind_rows(TableWrangle(LARC), TableWrangle(Tablet), TableWrangle(EmergencyContraception))
TableData%>% 
  gt() %>% 
  cols_hide(PercentageChange) %>% 
  tab_header(title = "Mean Quantities of Contraception Dispensed by NHS Scotland", subtitle = "Between October 2015 and July 2022") %>% 
  fmt_integer(MeanPrescriptionsDispensedPerHealthBoardPerTimePeriod) %>%
  cols_label(TimePeriod = "Time Period", TimePeriodDates = "Dates", MeanPrescriptionsDispensedPerHealthBoardPerTimePeriod = "Prescriptions Dispensed", PercentageChange = "Percentage Change") %>% 
  tab_row_group(label = "LARC", rows = 1:7) %>% 
  tab_row_group(label = "Oral", rows = 8:14) %>% 
  tab_row_group(label = "EC", rows = 15:21) %>% 
    cols_move(PercentageChangePretty, after = MeanPrescriptionsDispensedPerHealthBoardPerTimePeriod) %>%  cols_move(TimePeriodDates, after = TimePeriod) %>%
  tab_style(style = list(cell_text(color = "red")),
    locations = cells_body(columns = PercentageChangePretty, rows = PercentageChange < 0)) %>% 
    tab_style(style = list(cell_text(color = "chartreuse4")),
    locations = cells_body(columns = PercentageChangePretty, rows = PercentageChange > 0)) %>% 
  tab_style(locations = cells_column_labels(), style = list(cell_text(align = "center"))) %>% 
  gt_theme_538() %>% 
  tab_options(heading.align = "center", row_group.font.weight = "bold", row_group.background.color = "turquoise", row.striping.background_color = "grey90", column_labels.border.bottom.width = 0) %>% 
  opt_row_striping() %>% 
  cols_align(align = "center", columns = c(PercentageChangePretty, MeanPrescriptionsDispensedPerHealthBoardPerTimePeriod))
```

```{r ZoomedPlotting, warning=FALSE}
#SHAPE = 1 ERROR LOOK AT PDF  !!!!!!
ContraceptivesLogPlot <- Zoom(EmergencyContraception) %>%
  ...
  geom_line(aes(y = SumPaidQuantity), colour = "blue3") +
  geom_point(colour = "blue3", shape = 1) +
  
  geom_line(data = Zoom(Tablet),
            colour = "firebrick") +
  geom_point(data = Zoom(Tablet), colour = "firebrick", shape = 1) +
  
  geom_line(data = Zoom(LARC), colour = "purple") + 
  geom_point(data = Zoom(LARC), colour = "purple", shape = 1) +
...
LARCZoomPlot <- Zoom(LARC) %>%
  ...
  geom_point(colour = "purple", shape = 1) +
  scale_x_date(
...
TabletZoomPlot <- Zoom(Tablet) %>%
  ...
  geom_point(colour = "firebrick", shape = 1) +
  scale_y_continuous() +
  scale_x_date(
    date_breaks = "2 months",
    date_labels = "%b '%y",
...
ZoomedPlots <-
  wrap_plots(
    ContraceptivesLogPlot,
    TabletZoomPlot / LARCZoomPlot / EmergencyContraceptionZoomPlot
  ) +
